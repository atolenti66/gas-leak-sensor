...
const int bzr1 = 7;           // O número da porta para o Buzzer 1
...
void setup()
{
  ...
  pinMode(bzr1, OUTPUT);              //Declara que a porta do Buzzer1 é saída
  digitalWrite(bzr1,LOW);             //Para Buzzer 1
...
if (OperationState == 0){
   Serial.print("öperationstate = ");Serial.println(OperationState);
  ...
  Serial.println("Então beep...");
  bzrbeep(800);                              // Ativa o sinal sonoro com beep
}
...
void bzrbeep(int tempobeep){
  digitalWrite(bzr1,HIGH);               // Ativa Buzzer 1
  delay(tempobeep);
  digitalWrite(bzr1,LOW);               //Para Buzzer 1
  delay(tempobeep);
}
